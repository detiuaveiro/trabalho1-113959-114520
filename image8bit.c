...
Image ImageCreate(int width, int height, uint8 maxval) { ///
    ...
    // Preenche os campos da estrutura image
    img->width = width;
    img->height = height;
    img->maxval = maxval;
    ...
    if (img->pixel == NULL) {
        errCause = "Memory allocation error";
        free(img);  // Libera a memória alocada para a estrutura image
        return NULL;
    }

    // Inicializa os pixels (opcional)
...
void ImageDestroy(Image* imgp) { ///
    ...
    if (*imgp != NULL) {
        ...
        free(*imgp);           // Libera a memória alocada para a estrutura image
        *imgp = NULL;          // Define o ponteiro como NULL para evitar uso após a destruição
    }
}
...
Image ImageRotate(Image img) { ///
    ...
    Image rotated = ImageCreate(img->height, img->width, img->maxval);

    for (int y = 0; y < img->height; y++) {
        for (int x = 0; x < img->width; x++) {
            ImageSetPixel(rotated, y, img->width - x - 1, ImageGetPixel(img, x, y));
        }
    }
...
void ImagePaste(Image img1, int x, int y, Image img2) { ///
    ...
    //assert(ImageValidRect(dest, x, y, src->width, src->height));
    ...
    //        ImageSetPixel(dest, x + i, y + j, ImageGetPixel(src, i, j));
    //    }
    //}
//}
...
void ImageBlend(Image img1, int x, int y, Image img2, double alpha) { ///
    ...
    //        ImageSetPixel(img1, x + i, y + j, (uint8)(pixel1 * (1 - alpha) + pixel2 * alpha));
    //    }
    //}
    ...
    //for (int j = 0; j < src->height; j++) {
    //    for (int i = 0; i < src->width; i++) {
    //        uint8 blended = (ImageGetPixel(dest, x + i, y + j) + ImageGetPixel(src, i, j)) / 2;
    //        ImageSetPixel(dest, x + i, y + j, blended);
    //    }
    //}
//}
    ...
    for (int j = 0; j < img2->height; j++) {
        for (int i = 0; i < img2->width; i++) {
            if (ImageGetPixel(img1, x + i, y + j) != ImageGetPixel(img2, i, j)) {
                return 0; // Pixel mismatch
            }
        }
    }

    return 1; // Subimage found at (x, y)
}


    //for (int y = 0; y <= img->height - subimg->height; y++) {
    //    for (int x = 0; x <= img->width - subimg->width; x++) {
    //        if (ImageLocateSubImage(img, subimg, x, y)) {
...
int ImageLocateSubImage(Image img1, int* px, int* py, Image img2) { ///
    ...
    //        if (ImageGetPixel(img, x + i, y + j) != ImageGetPixel(subimg, i, j)) {
    //            return 0; // Pixel mismatch
    //        }
    //    }
    //}
